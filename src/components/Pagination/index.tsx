    ...
    <div className="flex-1 flex justify-between sm:hidden">
      {hasPrevious ? (
        <PaginationLink page={page - 1} query={query}>
          <div className="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white hover:text-gray-500 transition ease-in-out duration-150">
            Previous
          </div>
        </PaginationLink>
      ) : (
      ...
      {hasNext ? (
        <PaginationLink page={page + 1} query={query}>
          <div className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white hover:text-gray-500 transition ease-in-out duration-150">
            Next
          </div>
        </PaginationLink>
      ) : (
        <div className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white opacity-50">
          Next
        </div>
      )}
    </div>
...
function ResultDetails({ page, totalPages, totalItems }: ResultDetailsProps) {
  ...
  const last =
    page === totalPages ? totalItems : (page - 1) * itemsPerPage + itemsPerPage
